...
export default function Notes() {
  ...
  // New useEffect to open PDF in new tab when notes are loaded for selected subject
  useEffect(() => {
    if (notes.length > 0) {
      const firstNote = notes[0];
      if (firstNote && firstNote.filePath) {
        const apiBaseUrl = baseApi.replace("/api", "");
        const pdfUrl = `${apiBaseUrl}/uploads/${firstNote.filePath}`;
  ...
  const handleUpload = () => {
    ...
    formData.append("note", uploadFile);
    formData.append("subjectId", selectedSubjectId);

    axios
      .post(`${baseApi}/api/note/upload`, formData, {
        headers: {
      ...
      .catch((err) => {
        handleAxiosError(err);
        setUploadError("Error uploading note.");
      })
      .finally(() => {
        setUploading(false);
      });

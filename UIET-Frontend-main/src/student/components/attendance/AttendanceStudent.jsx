  ...
  const fetchStudentSubjects = async () => {
    try {
      ...
      if (
        ...
        profileRes.data.student.student_class
      ) {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      const res = await axios.get(
        `${baseApi}/api/attendance/student/${studentId}${queryString}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (res.data.success) {
        ...
        if (params.subject) {
          filteredData = filteredData.filter(
            (record) =>
              record.subjectName ===
              subjects.find((s) => s._id === params.subject)?.subject_name
          );
        }
        if (params.date) {
          filteredData = filteredData.filter(
            (record) =>
              new Date(record.date).toISOString().split("T")[0] === params.date
          );
        }
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2, boxShadow: 3, mb: 3, textAlign: "center" }}>
            <Typography variant="h6">
              Overall Attendance: {presentPercentage}%
            </Typography>
            <Typography>
              Present: {attendanceData[0].value} | Absent:{" "}
              {attendanceData[1].value} | Total Classes: {totalClasses}
            </Typography>
          </Paper>
        </Grid>
